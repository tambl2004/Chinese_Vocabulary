      ...
      <div className="flex-1 flex items-center justify-center p-4">
        {hasCompleted ? (
          ...
          <div className="flex flex-col items-center max-w-lg w-full">
                ...
                {showAnswer ? (
                  <div className="w-full border-t border-slate-100 pt-6 animate-in fade-in slide-in-from-bottom-2 duration-200 text-center">
                    <div className="mb-2">
                      ...
                      <span className="text-lg font-bold text-text-charcoal tracking-wide">{currentWord.pinyin}</span>
                    </div>
                    <div className="border-t border-slate-100/60 my-4" />
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="text-xs font-semibold text-text-muted uppercase tracking-wider block mb-0.5">Hán Việt</span>
                        <span className="text-sm font-semibold text-text-charcoal">{currentWord.han_viet}</span>
                      </div>
                      <div>
                        <span className="text-xs font-semibold text-text-muted uppercase tracking-wider block mb-0.5">Nghĩa</span>
                        <span className="text-sm font-semibold text-text-charcoal">{currentWord.meaning}</span>
                      </div>
                    </div>
                  </div>
                ) : (
            ...
            {showAnswer && (
              <div className="w-full grid grid-cols-3 gap-3 animate-in fade-in slide-in-from-bottom-3 duration-300">
                <button
                  onClick={() => handleUpdate('Chưa nhớ')}
                  disabled={isUpdating}
                  className="py-3 px-2 bg-status-red-bg border border-red-200 text-status-red-text font-bold text-xs rounded-lg hover:shadow-sm transition-all duration-150 hover:scale-[1.01]"
                >
                  Chưa nhớ
                </button>
                <button
                  ...
                  disabled={isUpdating}
                  className="py-3 px-2 bg-status-yellow-bg border border-amber-200 text-status-yellow-text font-bold text-xs rounded-lg hover:shadow-sm transition-all duration-150 hover:scale-[1.01]"
                >
                  Đang nhớ
                </button>
                <button
                  ...
                  disabled={isUpdating}
                  className="py-3 px-2 bg-status-green-bg border border-teal-200 text-status-green-text font-bold text-xs rounded-lg hover:shadow-sm transition-all duration-150 hover:scale-[1.01]"
                >
                  Đã nhớ
                </button>
              </div>
